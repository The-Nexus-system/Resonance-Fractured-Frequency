# Audio Bible

This document defines sound as the primary renderer of the Resonance world. Audio is gameplay. Every sound communicates information, and every sound should teach something.

## Audio is the primary renderer

Resonance is not "an audio game." It is a science-fiction game whose primary rendering engine is sound. Visuals enhance the experience but are never required. The world must be fully understandable through sound alone.

## Perspective audio

Every profession hears the same event differently. The sound mix changes, the dialogue changes, the vocabulary changes, and the way the AI communicates changes. The player should eventually be able to recognise another player's profession simply by listening.

Examples of what each profession hears follow.

- Medic: pulse, respiration, blood loss, triage, shock, evacuation.
- Engineer: power routing, reactor load, heat, pressure, hull stress.
- Scientist: energy signatures, anomalies, signal analysis, artefact behaviour.
- Security: enemy movement, weapon reports, cover.
- Officer: crew status, chain of command, evacuation priorities.
- Maintenance: vents, access panels, ducts, backup systems.
- Civilian: fear, confusion, public announcements, family.

## Dynamic soundscape

Sound is gameplay, so every sound communicates information. Examples of meaningful sounds include the following.

- Footsteps
- Hull vibration
- Air leaks
- Electrical failures
- Weapons
- Radio traffic
- Engines
- Alien wildlife
- Machines

Every sound should teach the player something about the world or the situation.

## Spatial audio

The game uses three-dimensional spatial audio. Turning the player's head continuously updates the spatial mix so the player can locate sources, orient themselves, and navigate by listening. Combat and flight both depend on spatial audio, as described in the Systems Bible.

## Adaptive music

Music reacts to the situation and the player's state. It responds to the following.

- Danger
- Hope
- Loss
- Victory
- Trust
- AI integration
- Discovery
- Psychological state
- Wonder

The soundtrack tells part of the story rather than only setting a mood.

## Audio and accessibility

Important information must never depend on sound alone. For players with no auditory input, urgency and information are conveyed through captions, visual cues, haptics, and an authored tactile language. Haptics are designed as an independent communication channel rather than a direct conversion of music into vibration; music may influence tactile patterns but does not define them. The Accessibility Bible defines the tactile language and these output paths in full.

## Everything has an acoustic identity

Every object, creature, environment, vehicle, tool, article of clothing, building, and technology in the Resonance universe possesses a deliberate acoustic identity, and silence is one of those identities. Audio is never decorative. It is part of the world's physical reality, and every sound communicates information.

If a player closes their eyes, they should still be able to learn where they are, what is happening, who is nearby, what tools are in use, what vehicle is approaching, and what emotional state the environment conveys. The libraries that follow apply this rule to every recurring category of sound in the franchise. The deliberate use of quiet is governed by the canon rule that silence is a deliberate storytelling tool.

Every sound in every library below is designed to serve a consistent set of purposes.

- Gameplay, because the sound advances or informs play
- Navigation, because the sound helps players build and hold a mental map
- Recognition, because the sound identifies what is present before it is seen
- Accessibility, because the sound has a defined non-auditory alternative
- Identity, because the sound distinguishes one thing, person, or place from another
- Emotional storytelling, because the sound carries feeling as well as fact

The acoustic identity of alien wildlife, alien technology, alien and Shawabti materials, and Shawabti culture is revealed progressively through play and recorded where the campaign reveals it. It is not specified in advance, so that the deliberate mysteries of the universe remain intact.

## Movement sound library

Movement is gameplay. How a character moves tells the player who is nearby, how they feel, what surface they are crossing, and how the surrounding space is shaped. Movement sound lets players build and maintain a mental map without sight, and it carries identity, because two people rarely move in exactly the same way.

### What each movement profile defines

- Normal sound
- Fast movement
- Slow movement
- Environmental interaction
- Surface interaction
- Accessibility considerations
- Recognition cues
- Navigation value
- Gameplay value
- Emotional storytelling

### Movements this library covers

- Walking
- Running
- Jogging
- Standing
- Shifting weight
- Turning
- Sitting
- Standing up
- Crawling
- Climbing
- Ladders
- Ramps
- Stairs
- Jumping
- Sliding
- Falling
- Swimming
- Floating in zero gravity
- Magnetic boots
- Spacewalks
- Transfers
- Manual wheelchair propulsion
- Power wheelchair propulsion
- Mobility scooters
- Walkers
- White cane travel
- Support canes
- Forearm crutches
- Platform crutches
- Prosthetic limbs
- Exoskeletons
- Guide animals
- Service animals
- Robotic mobility assistance
- Children's movement
- Older adult movement
- Fatigue
- Injury
- Medical gait changes
- Personal walking styles
- Personal running styles
- Personality expressed through movement

### Surfaces and floor materials

Movement sound changes with the surface underfoot, and the surface itself becomes a navigation cue. The acoustic properties of these and other substances are defined in the material library.

- Metal
- Carpet
- Concrete
- Wood
- Glass
- Grating
- Tile
- Soil
- Grass
- Sand
- Snow
- Ice
- Mud
- Water

### Acoustic environments for movement

Each area carries its own acoustic identity, so the same footsteps read differently depending on where they fall. The Modular Architecture Bible describes how these spaces are built, and the canon rule that everything has an acoustic identity governs how each is heard.

- Maintenance tunnels
- Engineering
- Medical
- Civilian housing
- Hydroponics
- Exterior settlements

## Vehicle sound library

Vehicles move people and cargo through the world, and each has an identity a player can recognise before it comes into view. Spacecraft classes are detailed in the spacecraft sound library; this library covers surface, station, and short-range transportation.

### What each vehicle profile defines

- Idle
- Startup
- Shutdown
- Acceleration
- Deceleration
- Turning
- Doors
- Passenger alerts
- Accessibility announcements
- Emergency operation
- Mechanical failure
- Exterior sound
- Interior sound
- Environmental interaction
- Recognition cues
- Blind navigation
- Passenger experience
- Crew experience
- Gameplay purpose

### Vehicles this library covers

- Escape pods
- Shuttles
- Maintenance carts
- Cargo vehicles
- Forklifts
- Medical transport
- Construction equipment
- Wheelchairs
- Power chairs
- Transit systems
- Elevators
- Trams
- Planetary vehicles
- Exploration craft
- Dropships
- Docking systems
- Airlocks
- Research vessels
- Capital ships
- Civilian spacecraft
- Emergency response vehicles
- Service robots
- Autonomous delivery systems

## Spacecraft sound library

Every class of spacecraft possesses an immediately recognisable acoustic identity, so a player can tell what has arrived, powered up, or begun to fail without seeing it. Spatial audio during flight is described in the Systems Bible.

### Spacecraft classes this library covers

- Escape pods
- Life pods
- Shuttles
- Dropships
- Construction craft
- Maintenance craft
- Research craft
- Medical craft
- Cargo craft
- Civilian transports
- Military vessels
- Capital ships
- Orbital stations

### Spacecraft systems and states this library defines

- Docking collars
- Docking clamps
- Airlocks
- Launch systems
- Recovery systems
- Emergency separation
- Hull damage
- Power-up
- Power-down
- Life support
- Navigation
- Bridge ambience
- Engineering ambience
- Crew quarters

## Space suit sound library

A space suit is a small, mobile environment, and the player should learn to recognise a suit and its condition without seeing it. Suit warnings, haptic alerts, and accessibility alerts follow the non-auditory paths defined in the Accessibility Bible.

### Suits this library covers

- Standard extravehicular suits
- Maintenance suits
- Engineering suits
- Medical biohazard suits
- Research suits
- Construction suits
- Fire suppression suits
- Planetary exploration suits
- Pressure suits
- Emergency survival suits

### What each suit profile defines

- Breathing
- Life support
- Cooling systems
- Joint movement
- Magnetic boots
- Tool attachment
- Radio communication
- Warning systems
- Damage
- Leaks
- Pressure loss
- Helmet sounds
- Visor interaction
- Haptic alerts
- Accessibility alerts
- Suit startup
- Suit shutdown
- Emergency operation

## Equipment sound library

Every recurring tool and accessibility device has a recognisable sound identity, so the player knows what is in use before seeing it. Accessibility devices are ordinary equipment and sound like it, as the Accessibility Bible describes. Personal beacons, digital identity badges, and smart name tapes are part of the layered crew identification system detailed in the Universe Bible and the Player Identity and Character Presentation Bible.

### What each equipment profile defines

- Activation
- Idle
- Use
- Completion
- Warnings
- Errors
- Accessibility alternatives
- Recognition cues
- Gameplay purpose
- Navigation value
- Emotional storytelling

### Equipment this library covers

- Augmentative and alternative communication devices
- Refreshable braille displays
- Braille note takers
- Screen readers
- Hearing aids
- Cochlear implants
- White canes
- Smart canes
- Medical scanners
- Engineering tools
- Maintenance tools
- Fabricators
- Research equipment
- Security equipment
- Communicators
- Personal beacons
- Digital identity badges
- Smart name tapes
- Doors
- Control panels
- Consoles
- Wearable devices
- Protective equipment
- Environmental sensors
- Laboratory equipment
- Diagnostic equipment
- Portable power tools
- Inspection equipment

## Weapon sound library

Weapons exist primarily as emergency tools, and they should sound purposeful rather than exciting. The audio philosophy here emphasises responsibility over spectacle, consistent with the tone of the franchise.

### Emergency tools this library covers

- Rescue equipment
- Non-lethal equipment
- Security equipment
- Training equipment
- Industrial cutting tools
- Emergency breaching tools
- Personal defence equipment
- Historic weapons
- Alien technology
- Experimental systems

### What each emergency tool profile defines

- Draw
- Ready
- Safety
- Activation
- Discharge
- Reload
- Power cycle
- Failure
- Overheating
- Cooling
- Maintenance
- Accessibility alternatives
- Narrative purpose

## Clothing sound library

Clothing contributes to recognition, and every recurring character possesses recognisable clothing sounds that support the recognition cues defined in the Player Identity and Character Presentation Bible.

### Clothing this library covers

- Command uniforms
- Engineering uniforms
- Maintenance uniforms
- Medical uniforms
- Research uniforms
- Security uniforms
- Civilian clothing
- Children's clothing
- Protective equipment
- Environmental gear
- Jewellery
- Identification badges
- Mobility equipment attached to clothing

## Material library

Every interaction inherits its sound from the material involved, so the same action reads differently on steel than on fabric. Movement across these materials is covered in the movement sound library.

### Materials this library covers

- Steel
- Titanium
- Composite materials
- Ceramics
- Fabric
- Leather
- Glass
- Transparent aluminium
- Stone
- Wood
- Ice
- Water
- Sand
- Soil
- Plants
- Alien materials
- Shawabti materials

## Object sound library

Every recurring interactive object possesses a defined sound identity and provides meaningful acoustic feedback, so the player can confirm an action by ear.

### Objects this library covers

- Doors
- Windows
- Storage
- Furniture
- Medical beds
- Hospital equipment
- Kitchen equipment
- Fabricators
- Lighting systems
- Terminals
- Consoles
- Books
- Paper
- Tools
- Containers
- Laboratory equipment
- Maintenance panels
- Elevators
- Ladders
- Handrails
- Control surfaces
- Emergency systems

## Animal sound library

Animals have their own acoustic identities, and the player can recognise a companion, a working animal, or nearby wildlife by ear. The species themselves are defined in the Universe Bible.

### What each animal profile defines

- Daily sounds
- Movement sounds
- Emotional sounds
- Warning sounds
- Environmental interaction
- Recognition cues
- Accessibility value
- Gameplay value

### Animals this library covers

- Companion animals
- Service animals
- Working animals
- Native wildlife
- Alien wildlife
- Environmental insects
- Aquatic life
- Birds
- Urban wildlife
- Agricultural animals

## Biological sound library

The sounds produced by living beings contribute to realism, empathy, and accessibility, and they let the player sense health, effort, and emotion without sight.

### Biological sounds this library covers

- Breathing
- Heartbeat
- Speech
- Signing interaction
- Augmentative and alternative communication
- Eating
- Drinking
- Sleeping
- Fatigue
- Pain
- Recovery
- Laughter
- Crying
- Celebration
- Work
- Exercise
- Medical examination
- Children
- Older adults

## Environmental sound progression

Locations evolve acoustically over time, and the player should hear civilisation evolving. This section applies the canon rule that sound is progression, and it works alongside the Crew Progression System and the Modular Architecture Bible, so that the community can be heard growing before it is seen.

### How locations evolve acoustically

- Construction progresses
- Gardens mature
- Schools become active
- Communities grow
- Repairs finish
- Neighbourhoods develop
- Stations recover
- Settlements expand

## Cultural sound identity

Every civilisation possesses recognisable sound characteristics, and no civilisation sounds identical to another. The civilisations themselves and their cultural frameworks are defined in the Universe Bible, and the acoustic identity of the Shawabti and of any culture the player has not yet encountered is revealed progressively through play rather than fixed here.

### What gives a civilisation its sound

- Languages
- Greetings
- Public announcements
- Music
- Celebrations
- Ceremonies
- Markets
- Schools
- Religious or philosophical gatherings
- Emergency broadcasts
- Public transportation
- Workplaces
- Community events
- Accessibility systems

## Emotional sound design

Sound communicates emotion without relying solely on music, so a player who cannot hear the score still receives the feeling through ambience, movement, biological sound, and the tactile and captioned paths defined in the Accessibility Bible.

### Emotional states sound communicates

- Safety
- Danger
- Hope
- Recovery
- Loss
- Celebration
- Discovery
- Curiosity
- Belonging
- Isolation
- Memory
- Resilience

## The definitive sound reference

The Audio Bible is the definitive reference for every sound in the Resonance franchise. No sound exists without purpose, and every sound contributes to the goals the franchise sets for its audio.

- Gameplay
- Accessibility
- Navigation
- Identity
- World-building
- Community
- Memory
- Emotional storytelling

## Relationship to other documents

This document is the definitive reference for sound in the franchise, and it elaborates the canon rules that everything has an acoustic identity, that silence is a deliberate storytelling tool, and that sound is progression.

- The Canon Rules document holds the master wording of the audio-related rules this document elaborates.
- The Accessibility Bible defines the tactile language, captions, haptics, and other non-auditory paths that every accessibility alternative in these libraries relies on.
- The Modular Architecture Bible describes how the spaces heard in these libraries are built and how each carries its own acoustic identity.
- The Player Identity and Character Presentation Bible defines the recognition cues, clothing and movement sound, and the personal beacon, digital identity badge, and smart name tape referenced here.
- The Systems Bible describes how spatial audio is used in combat and flight and how mechanics are introduced through sound.
- The Universe Bible is the definitive reference for the species, civilisations, wildlife, cultural frameworks, and identification technology whose sounds these libraries catalogue, and it records the Shawabti and other deliberate mysteries whose acoustic identity is revealed through play.
- The Crew Progression System design note describes how the community is heard growing as crew are rescued.
- The Decision Log records when the audio-related rules were adopted or amended.
