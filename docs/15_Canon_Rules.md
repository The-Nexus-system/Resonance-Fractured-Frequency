# Canon Rules

This document is the consolidated, authoritative list of canon rules for Resonance. A canon rule is a promise the game keeps to its players and a constraint every chapter, system, and profession must honor.

These rules apply across the whole franchise. Where another document elaborates a rule in detail, this document remains the single place to see every rule at once. If a design conflicts with a rule here, the design changes, or the rule is formally amended and the change is recorded in the Decision Log. The Project Constitution has authority over this document.

Each rule below has a descriptive heading so it can be found and cited directly.

## Chapters Grow Gameplay and Story Together

Every chapter must introduce one major gameplay system.

Every chapter must answer one major narrative question.

Every chapter must end by asking a larger question than it answered.

Gameplay progression and narrative progression must grow together.

The player should feel they are becoming more capable for the same reason the story is becoming more complex.

## Operation One Happens Once, Seen Through Many Perspectives

Operation One exists only once.

Every profession experiences the same historical events.

No profession changes history.

Only perspective changes.

The player gains a fuller understanding of the story by replaying different professions.

Events that appear mysterious from one perspective become understandable from another.

All profession campaigns must remain synchronized to the same timeline.

## A Campaign Is Complete Only When Every Profession Exists

A campaign is not complete until every primary profession has been fully implemented.

Maintenance is developed first.

The remaining professions are then built upon the same historical timeline.

Each profession contributes different gameplay, different knowledge, different responsibilities and different emotional experiences.

## People Introduce Mechanics, Not Menus

Every gameplay mechanic must be introduced by a person before it becomes a player ability.

Players learn from people.

Not menus.

## Accessibility Is Standard Equipment

No accessibility feature is considered special equipment.

Accessibility technology is standard equipment aboard human vessels.

Crew members customize interfaces rather than requesting permission to use them.

## Every Location Has an Acoustic Identity

Every major location has an identifiable acoustic identity.

Players should eventually recognize locations by sound before dialogue confirms where they are.

## Silence Is a Deliberate Storytelling Tool

Silence is a deliberate storytelling tool.

Silence should communicate emotion, danger, absence, distance or anticipation.

Silence should never exist simply because audio has not yet been created.

## Failure Changes History Rather Than Restarting It

Failure changes history.

Failure does not normally restart history.

Players recover from mistakes whenever narratively reasonable.

Consequences become part of the continuing story.

## Knowledge Must Always Change Something

Knowledge never exists only as collectible lore.

Every Knowledge Archive entry must eventually change gameplay, dialogue, exploration, diplomacy, accessibility, relationships or player understanding.

If knowledge changes nothing, it should not exist.

## Everyone Teaches and Everyone Keeps Learning

Every crew member teaches.

Every crew member continues learning.

Expertise is shared throughout the community.

No crew member knows everything.

## The Player Is Never the Smartest Person in the Room

The player is never the smartest person in the room.

Every crew member possesses expertise unavailable to the player.

The player's greatest strength is bringing expertise together.

## The Home Is Always Evolving

The player's home is always evolving.

Rooms change.

Soundscapes change.

Relationships change.

Daily routines change.

The station continues living even when the player is elsewhere.

## History and Understanding Are Earned Separately

The player's actions create history.

The player's observations create understanding.

History and understanding are not always acquired at the same time.

## Every Chapter Leaves the Home Better

Every chapter should leave the player's home better than it was before.

Even during tragedy.

Even during loss.

Humanity continues building.

Hope is measured by what people create together.

## No Primary Crew Member Becomes Obsolete

No primary crew member ever becomes obsolete.

Every primary crew member remains mechanically and emotionally relevant throughout the entire campaign.

Every chapter should provide meaningful opportunities for every primary crew member to contribute according to their expertise.

People remain progression from the first rescue through the final chapter.

## Portraits Represent Belonging, Not Recruitment

A rescued crew member is not considered home immediately after rescue.

A crew member becomes home only after they unpack, settle, begin contributing and change the environment around them.

Only then does their portrait illuminate.

Portraits represent belonging, not recruitment.

## Sound Is Progression

Sound is progression.

The player should hear the community growing before receiving visual confirmation.

Every rescued crew member permanently changes the station's soundscape.

Visual progression follows acoustic progression.

## Community Is Progression

Community is progression.

People, not equipment, are the primary progression system throughout Resonance.

Technology supports people.

People give technology purpose.

## The Station Grows Through People Working Together

The station never grows because the player collected enough resources alone.

The station grows because people work together.

Major construction projects should require multiple crew members with different expertise.

## Perspectives Complete Each Other Without Contradiction

No perspective may contradict another perspective.

Different professions reveal different pieces of the same historical events.

Together they create a complete historical record.

No single profession tells the entire truth.

## Every Profession Answers Three Questions Differently

Every profession should answer three questions differently.

- What does this profession notice?
- What responsibilities does this profession carry?
- What knowledge is available only from this perspective?

## Every Major Decision Answers Four Questions

Every major section that presents a player decision must answer four questions.

- What decision does the player make?
- What gameplay system is learned?
- What philosophy or life lesson is learned?
- What permanently changes because of that decision?

This gives each section something concrete to build rather than atmosphere alone, and it keeps player choice meaningful even when the main story stays on the same timeline.

## Every System Reinforces the Central Themes

Every gameplay system should reinforce the central themes of Resonance.

- Community.
- Accessibility.
- Curiosity.
- Adaptation.
- Shared knowledge.
- Hope.

If a mechanic reinforces none of these themes, it should be reconsidered.

## Every Major Expedition Begins and Ends at Home

The player's home is the emotional, mechanical, and narrative anchor of Resonance. Every expedition follows the same rhythm.

- Prepare.
- Depart.
- Explore.
- Recover.
- Return.
- Reflect.
- Rebuild.

The player should always return home changed, and the station should always change because of what the player brought home. The player never returns with resources alone. Every expedition must return with at least one meaningful contribution to the community.

Meaningful contributions include:

- A survivor.
- Knowledge.
- History.
- Technology.
- Cultural artifacts.
- Accessibility improvements.
- Engineering discoveries.
- Medical discoveries.
- Relationships.
- Hope.

Resources support these discoveries. Resources are never the primary reward.

## Every Expedition Expands Understanding

Every expedition must answer one important question, and every expedition must create at least two new questions. Understanding should continually expand, and curiosity should always outpace certainty. The player should finish every expedition knowing more than before while realizing there is still far more left to discover.

## Relationship to Other Documents

This document is the master list of canon rules and holds precedence for the wording of a rule. Several rules are elaborated in depth elsewhere, and those documents remain the place for detail while this document remains the place for the complete set. Where another document restates a rule, that restatement is explanatory, not a second source of truth; if the wording drifts, the statement here governs. Only the Project Constitution has authority over this document.

- The Project Constitution defines the core philosophy these rules serve, and has authority over this document.
- The Campaign Bible elaborates the chapter and Operation One perspective rules.
- The Systems Bible elaborates how mechanics are introduced and how systems reinforce the themes.
- The Knowledge Archive Bible elaborates how knowledge changes gameplay.
- The Audio Bible elaborates acoustic identity, silence, and sound as progression.
- The Accessibility Bible elaborates accessibility as standard equipment.
- The Crew Progression System design note elaborates the crew relevance, belonging, sound, and community rules.
- The Decision Log records when each rule was adopted or amended.

Whenever a rule here changes, the elaborating documents must be checked for consistency.
